...
const contract_address = "0x7743725d13f7f684988488786dE0397CB075F222"; // 따옴표 안에 주소값 복사 붙여넣기
...
const _shareRoom = async (name, location, price) => {
  ...
  // 화면을 업데이트 한다.
  //let user = document.getElementById("address").value;
  try{
	  let rooms = await getRoomShareContract().then(app => app.methods.shareRoom(name, location, price).send({from:user, gas: 3000000}));
  	  alert("Registration!");
  	  _updateRooms();
  }catch(err){
	console.log(err);
...
const _getMyRents = async () => {
  // 내가 대여한 방 리스트를 불러온다.
  let myRents = await getRoomShareContract().then(app => app.methods.getMyRents().call({from:user}));
  return myRents;
}
...
const _getAllRooms = async () => {
  ...
  let user = document.getElementById("address").value;
  let rooms = await getRoomShareContract().then(app => app.methods.getAllRooms().call({from:user}));
  return rooms;
}
...
const _rentRoom = async (roomId, currentYear, checkInDate, checkOutDate, price) => {
  ...
  try{
	let rent = await getRoomShareContract().then(app =>
          app.methods.rentRoom(roomId, currentYear, checkInDate, checkOutDate)
          .send({from:user, gas: 3000000, value: (Number(checkOutDate) - Number(checkInDate)) * Number(price)})).catch(err => {throw "error";});
	  alert("Success!");
  }catch(err){
	  let isInActive = await getRoomShareContract().then(app => app.methods.getIsActive(roomId).call({from:user}));
	  let isInvalidValue = await getRoomShareContract().then(app => app.methods.getIsInvalidValue((Number(checkOutDate) - Number(checkInDate)) * Number(price)).call({from:user}));
          let isAlreadyReservation = await getRoomShareContract().then(app => app.methods.getIsAlreadyReservation(roomId, checkInDate, checkOutDate).call({from:user}));
          if(isInvalidValue == true){
                        alert("Your balance is insufficient.");
          }
          else if(isAlreadyReservation == true){
                        _recommendDate(roomId, currentYear, checkInDate, checkOutDate);
          }
	  else if(isInActive == false){
		alert("That room is inactive.");
	  }
          else{
                        console.log(err);
          }
...
const _recommendDate = async (roomId, currentYear, checkInDate, checkOutDate) => {
  ...
  let recommendDate = await getRoomShareContract().then(app => app.methods.recommendDate(roomId, checkInDate, checkOutDate).call({from:user}));
  alert(dateFromDay(currentYear,recommendDate[0]).toDateString().concat("  ~  ").concat(dateFromDay(currentYear, recommendDate[1]).toDateString()).concat("  is already booked :("));
}
...
const getRoomRentHistory = async () => {
  ...
  let arr = [];
  let history = await getRoomShareContract().then(app => app.methods.getRoomRentHistory(returnOptionsJSON()[0]).call({from:user}));
  return history
}
...
const displayRoomHistory = async () => {
    ...
    html += "<td>" + dateFromDay(history[i].yearOfRent,history[i].checkInDate).toDateString() + "</td>"
    html += "<td>" + dateFromDay(history[i].yearOfRent,history[i].checkOutDate).toDateString() + "</td>"
...
const markRoomAsInactive = async () => {
  ...
  let inactive = await getRoomShareContract().then(app => app.methods.markRoomAsInactive(_roomId).send({from:user, gas:3000000}));
}
...
const intializeRoomShare = async () => {
  ...
  let _roomId = document.getElementById('roomId').value;
  let initialize = await getRoomShareContract().then(app => app.methods.initializeRoomShare(_roomId).send({from:user, gas:3000000}));
}
